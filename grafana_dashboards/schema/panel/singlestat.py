...
class Singlestat(Base):
    def get_schema(self):
        ...
        sparkline = {
            v.Required("fillColor", default=sparkline_defaults["fillColor"]): v.All(
                str
            ),
            v.Required("full", default=False): v.All(bool),
            v.Required("lineColor", default=sparkline_defaults["lineColor"]): v.All(
                str
            ),
        ...
        singlestat = {
            ...
            v.Required("postfixFontSize", default="50%"): v.All(
                v.Match(r"^[1-9]?[0]{1}%$|^1[0-9]?[0]{1}%$|^200%$")
            ),
            v.Required("prefix", default=""): v.All(str),
            # Support 0% to 200% by 10
            v.Required("prefixFontSize", default="50%"): v.All(
                v.Match(r"^[1-9]?[0]{1}%$|^1[0-9]?[0]{1}%$|^200%$")
            ),
            ...
            # Support 0% to 200% by 10
            v.Required("valueFontSize", default="80%"): v.All(
                v.Match(r"^[1-9]?[0]{1}%$|^1[0-9]?[0]{1}%$|^200%$")
            ),
